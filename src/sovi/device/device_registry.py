...
def get_active_devices() -> list[dict[str, Any]]:
    """Get all operational devices, ordered by name.
...
def to_wda_device(row: dict[str, Any]) -> WDADevice:
    ...
    name = row.get("name") or row["udid"][:12]
    wda_port = row.get("wda_port") or 8100
    return WDADevice(
        name=name,
        udid=row["udid"],
        wda_port=wda_port,
...
def update_heartbeat(device_id: UUID | str) -> None:
    """Update device connected_since timestamp."""
    sync_execute(
        "UPDATE devices SET connected_since = now(), updated_at = now(), status = 'active' WHERE id = %s",
        (str(device_id),),
    )
...
def generate_launchd_plists(device: dict[str, Any], output_dir: str | None = None) -> list[str]:
    ...
    name = (device.get("name") or "unknown").lower().replace(" ", "-")
    udid = device["udid"]
    wda_port = device.get("wda_port") or 8100
    generated = []
